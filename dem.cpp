...
public:
  ...
  void followLineWithGroundSensors() {
    ...
    // Adjust these thresholds based on your updated information
    const double WHITE_THRESHOLD = 5.0;   // White background
    const double GREEN_THRESHOLD = 7.0;   // Green background
    const double BASE_SPEED = MAX_SPEED * 0.4;
    const double TURN_SPEED = MAX_SPEED * 0.2;
    
    // Line following logic
    if (leftValue > GREEN_THRESHOLD && rightValue > GREEN_THRESHOLD) {
      // Both sensors on green line - go straight
      setMotorSpeeds(BASE_SPEED, BASE_SPEED);
    } else if (leftValue > GREEN_THRESHOLD && rightValue <= WHITE_THRESHOLD) {
      // Left sensor on green line, right sensor on white - turn left
      setMotorSpeeds(TURN_SPEED, BASE_SPEED);
    } else if (rightValue > GREEN_THRESHOLD && leftValue <= WHITE_THRESHOLD) {
      // Right sensor on green line, left sensor on white - turn right
      setMotorSpeeds(BASE_SPEED, TURN_SPEED);
    } else {
      // Both sensors on white - perform a tight turn to search for the line
  ...
  void run() {
    ...
    while (step(TIME_STEP) != -1) {
      if (!greenLineFound && searchAttempts < 4) {
        ...
        // Continue moving forward to push the cube
        setMotorSpeeds(MAX_SPEED * 0.5, MAX_SPEED * 0.5);
      }
    }
  }
